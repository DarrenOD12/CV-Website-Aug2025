...
const experiences = [
  ...
  },
  {
    ...
    achievements: [
      "Managed a high-stakes data migration and UAT project for a public sector client, successfully implementing a nationwide automated healthcare demand management system during COVID-19. Conducted enablement sessions for nationwide providers to ensure effective adoption.",
      "Orchestrated large-scale testing processes as a Test Management Lead for an international airline, enhancing system reliability and performance for their new ERP solution.",
  ...
  return (
    <div className="min-h-screen bg-background pt-8 pb-16 px-4">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-4xl font-bold text-center text-foreground mb-12 font-helixa">Experience</h2>
        <Accordion type="multiple" defaultValue={["item-0"]} className="space-y-4">
          {experiences.map((experience, index) => (
            <AccordionItem key={index} value={`item-${index}`} className="border-2 border-border rounded-lg bg-card">
              <AccordionTrigger className="px-6 py-4 hover:no-underline hover:bg-muted/50 rounded-t-lg data-[state=open]:rounded-b-none">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 w-full text-left">
                  <div className="flex-1">
                    ...
                    {experience.location && (
                      <div className="text-sm text-muted-foreground mt-1 font-helixa">{experience.location}</div>
                    )}
                  ...
                  <div className="text-right flex-shrink-0">
                    {experience.period && (
                      <div className="text-sm text-muted-foreground font-helixa">{experience.period}</div>
                    )}
                  </div>
                </div>
              </AccordionTrigger>
